...
namespace AdventOfCode
{
    ...
    public class Year2022Day17 : IAdventOfCode
    {
        ...
        public void GetSolution(string path, bool trackTime = false)
        {
            ...
            long result = new Tunnel(input, 100).AddRocks(2022).Height;

            _SW.Stop();

            Console.WriteLine(
                $"Part 1: Height after 2022 drop tetris blocks: {result}, Execution Time: {StopwatchUtil.getInstance().GetTimestamp(_SW)}");

            _SW.Restart();

            result = new Tunnel(input, 100).AddRocks(1_000_000_000_000).Height;

            _SW.Stop();

            Console.WriteLine(
                $"Part 2: Height after 1 trillion rocks: {result}, Execution Time: {StopwatchUtil.getInstance().GetTimestamp(_SW)}");
